...
import { match, stub, spy, assert } from 'sinon';
import MockRequest from 'readable-mock-req';
import { expect } from 'chai';

import { Issuer, Strategy } from '../../lib';
import instance from '../../lib/helpers/weak_cache';
...
describe('OpenIDConnectStrategy', () => {
  ...
  describe('authenticate', function () {
    it('forwards options.extras to callback as extras param', async function () {
      ...
      const req = new MockRequest('GET', '/login/oidc');
      req.session = { 'oidc:op.example.com': match.object };

      /* Fake callback params */
      const callbackParams = { code: 'some-code' };
      stub(this.client, 'callbackParams').callsFake(() => callbackParams);

      this.client.callback = spy();

      strategy.authenticate(req, {});
      assert.calledOnce(this.client.callback);
      assert.calledWith(
  ...
  describe('callback', function () {
    it('triggers the verify function and then the success one', function (next) {
      const ts = { foo: 'bar' };
      stub(this.client, 'callback').callsFake(async () => ts);
    ...
    it('triggers the error function when non oidc error is encountered', function (next) {
      ...
      stub(this.client, 'callback').callsFake(async () => {
    ...
    it('does userinfo request too if part of verify arity and resulting tokenset', function (next) {
      ...
      const ui = { sub: 'bar' };
      stub(this.client, 'callback').callsFake(async () => ts);
      stub(this.client, 'userinfo').callsFake(async () => ui);
