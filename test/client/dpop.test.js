...
const jwk = JWK.generateSync('RSA');
...
describe('DPoP', () => {
  ...
  describe('dpopProof', () => {
    ...
    it('must be passed a private key', function () {
      ...
      expect(() => this.client.dpopProof({}, JWK.generateSync('oct').toJWK(true))).to.throw(msg);
      expect(() => this.client.dpopProof({}, JWK.generateSync('EC').toJWK())).to.throw(msg);
      expect(() => this.client.dpopProof({}, JWK.generateSync('EC').toPEM())).to.throw(msg);
    });
    ...
    it('DPoP Proof JWT', function () {
      ...
      expect(
        JWT.decode(
          this.client.dpopProof({}, JWK.generateSync('EC')),
          { complete: true },
        ),
      ...
      if ('sign' in crypto) {
        expect(
          JWT.decode(
            this.client.dpopProof({}, JWK.generateSync('OKP')),
            { complete: true },
          ),
    ...
    it('key.alg is used if present', function () {
      const proof = this.client.dpopProof({}, JWK.generateSync('RSA', 2048, { alg: 'PS384' }));
      expect(JWT.decode(proof, { complete: true })).to.have.nested.property('header.alg', 'PS384');
    });

    it('else this.issuer.dpop_signing_alg_values_supported is used', function () {
      const proof = this.client.dpopProof({}, JWK.generateSync('RSA', 2048));
      expect(JWT.decode(proof, { complete: true })).to.have.nested.property('header.alg', 'PS512');
    });

    it('else key.algorithms("sign")[0] is used', function () {
      const proof = this.client.dpopProof({}, JWK.generateSync('EC'));
      expect(JWT.decode(proof, { complete: true })).to.have.nested.property('header.alg', 'ES256');
    });
  });
