...
import { reset, travel } from 'timekeeper';

import { Issuer } from '../../lib';
import DeviceFlowHandle from '../../lib/device_flow_handle';
...
describe('Device Flow features', () => {
  ...
  describe('DeviceFlowHandle', () => {
    ...
    it('the handle tracks expiration of the device code', () => {
      ...
      expect(handle.expired()).to.be.false;
      travel(new Date(Date.now() + 100 * 1000));
      expect(handle.expired()).to.be.false;
      travel(new Date(Date.now() + 200 * 1000));
