...
import MockRequest from 'readable-mock-req';
import { expect } from 'chai';
import { encode as _encode, decode } from 'base64url';
import nock, { cleanAll, isDone } from 'nock';
import { spy } from 'sinon';
import { JWT as _JWT, JWE, JWKS, JWS } from 'jose';
import { reset, freeze } from 'timekeeper';

import TokenSet from '../../lib/token_set';
import { OPError } from '../../lib/errors';
import now from '../../lib/helpers/unix_timestamp';
import { Registry, Issuer, custom } from '../../lib';
import { authFor } from '../../lib/helpers/client';

const fail = () => { throw new Error('expected promise to be rejected'); };
const encode = (object) => _encode(JSON.stringify(object));
...
describe('Client', () => {
  ...
  describe('#authorizationUrl', function () {
    ...
    it('returns a string with the url with some basic defaults', function () {
      expect(parse(this.client.authorizationUrl({
    ...
    it('returns a string with the url and client meta specific defaults', function () {
      expect(parse(this.clientWithMeta.authorizationUrl({
        nonce: 'foo',
      }), true).query).to.eql({
        nonce: 'foo',
        client_id: 'identifier',
    ...
    it('returns a string with the url and no defaults if client has more metas', function () {
      expect(parse(this.clientWithMultipleMetas.authorizationUrl(), true).query).to.eql({
        client_id: 'identifier',
        scope: 'openid',
      });
    });
    ...
    it('keeps original query parameters', function () {
      expect(parse(this.clientWithQuery.authorizationUrl({
        redirect_uri: 'https://rp.example.com/cb',
      }), true).query).to.eql({
        client_id: 'identifier',
        redirect_uri: 'https://rp.example.com/cb',
  ...
  describe('#endSessionUrl', function () {
    ...
    it('defaults the post_logout_redirect_uri if client has some', function () {
      expect(parse(this.clientWithUris.endSessionUrl(), true).query).to.eql({
    ...
    it('takes a TokenSet too', function () {
      ...
      expect(parse(this.client.endSessionUrl({
        id_token_hint: hint,
      }), true).query).to.eql({
    ...
    it('allows for recommended and optional query params to be passed in', function () {
      ...
      expect(parse(this.clientWithQuery.endSessionUrl({
        post_logout_redirect_uri: 'https://rp.example.com/logout/cb',
        state: 'foo',
        id_token_hint: 'idtoken',
  ...
  describe('#callback', function () {
    ...
    describe('jarm response mode', function () {
      ...
      it('consumes encrypted JARM responses', async function () {
        ...
        const response = JWE.encrypt(_JWT.sign({
          code: 'foo',
        }, client.client_secret, {
          issuer: this.issuer.issuer,
          audience: client.client_id,
  ...
  describe('#authFor', function () {
    describe('when none', function () {
      it('returns the body httpOptions', async function () {
        ...
        expect(await authFor.call(client, 'token')).to.eql({
    ...
    describe('when client_secret_post', function () {
      ...
      it('requires client_secret to be set', function () {
        ...
        return authFor.call(client, 'token')
          .then(fail, (error) => {
            expect(error).to.be.instanceof(TypeError);
    ...
    describe('when client_secret_jwt', function () {
      before(function () {
        ...
        return Promise.all([
          authFor.call(client, 'token').then((auth) => { this.auth = auth; }),
          authFor.call(client, 'token', { clientAssertionPayload: { aud: 'https://rp.example.com' } }).then((auth) => { this.authWithClientAssertionPayload = auth; }),
        ]);
      });
      ...
      it('has a predefined payload properties', function () {
        const payload = JSON.parse(decode(this.auth.form.client_assertion.split('.')[1]));
      ...
      it('can use clientAssertionPayload to change the default payload properties', function () {
        const payload = JSON.parse(decode(this.authWithClientAssertionPayload.form.client_assertion.split('.')[1]));
        expect(payload).to.have.keys(['iat', 'exp', 'jti', 'iss', 'sub', 'aud']);
      ...
      it('has the right header properties', function () {
        const header = JSON.parse(decode(this.auth.form.client_assertion.split('.')[0]));
  ...
  describe('Distributed and Aggregated Claims', function () {
          ...
          nock('https://src3.example.com')
            ...
            .get('/claims').reply(200, [{ alg: 'none' }, { age: 27 }, ''].map((comp) => {
              if (typeof comp === 'object') {
                return _encode(JSON.stringify(comp));
              }
              return comp;
            }).join('.'));
    ...
    describe('private #decryptIdToken', function () {
      ...
      it('verifies the id token is using the right alg', async () => {
        ...
        const header = _encode(JSON.stringify({
          alg: 'RSA1_5',
          enc: 'A128CBC-HS256',
        }));
    ...
    describe('#callbackParams', function () {
      ...
      describe('when http.IncomingMessage', () => {
        before(function () {
          this.origIncomingMessage = IncomingMessage;
          IncomingMessage = MockRequest;
        });

        after(function () {
          IncomingMessage = this.origIncomingMessage;
        });
    ...
    describe('#requestObject', function () {
      ...
      it('sign alg=none', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((signed) => {
            ...
            expect(JSON.parse(decode(parts[0]))).to.eql({ alg: 'none', typ: 'oauth-authz-req+jwt' });
            const {
              jti, iat, exp, ...jwt
      ...
      it('sign alg=HSxxx', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((signed) => {
            const parts = signed.split('.');
            expect(JSON.parse(decode(parts[0]))).to.eql({ alg: 'HS256', typ: 'oauth-authz-req+jwt' });
            const {
              jti, iat, exp, ...jwt
            } = JSON.parse(decode(parts[1]));
            expect(jwt).to.eql({
      ...
      it('sign alg=RSxxx', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((signed) => {
            const parts = signed.split('.');
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'RS256', typ: 'oauth-authz-req+jwt' }).and.have.property('kid');
            const {
              jti, iat, exp, ...jwt
            } = JSON.parse(decode(parts[1]));
            expect(jwt).to.eql({
      ...
      it('encrypts for issuer using issuer\'s public key (explicit enc)', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((encrypted) => {
            ...
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'RSA1_5', enc: 'A128CBC-HS256', cty: 'oauth-authz-req+jwt' }).and.have.property('kid');
          });
      });
      ...
      it('encrypts for issuer using pre-shared client_secret (A\\d{3}GCMKW)', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((encrypted) => {
            const parts = encrypted.split('.');
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'A128GCMKW', enc: 'A128CBC-HS256', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
          });
      });
      ...
      it('encrypts for issuer using pre-shared client_secret (dir + A128CBC-HS256)', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((encrypted) => {
            const parts = encrypted.split('.');
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'dir', enc: 'A128CBC-HS256', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
          });
      });
      ...
      it('encrypts for issuer using pre-shared client_secret (dir + A192CBC-HS384)', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((encrypted) => {
            const parts = encrypted.split('.');
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'dir', enc: 'A192CBC-HS384', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
          });
      });
      ...
      it('encrypts for issuer using pre-shared client_secret (dir + A256CBC-HS512)', function () {
        ...
        return client.requestObject({ state: 'foobar' })
          .then((encrypted) => {
            const parts = encrypted.split('.');
            expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'dir', enc: 'A256CBC-HS512', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
          });
      });
      ...
      if (!('electron' in process.versions)) {
        it('encrypts for issuer using pre-shared client_secret (PBES2)', function () {
          ...
          return client.requestObject({ state: 'foobar' })
            .then((encrypted) => {
              const parts = encrypted.split('.');
              expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'PBES2-HS256+A128KW', enc: 'A128CBC-HS256', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
            });
        });
        ...
        it('encrypts for issuer using pre-shared client_secret (A\\d{3}KW)', function () {
          ...
          return client.requestObject({ state: 'foobar' })
            .then((encrypted) => {
              const parts = encrypted.split('.');
              expect(JSON.parse(decode(parts[0]))).to.contain({ alg: 'A128KW', enc: 'A128CBC-HS256', cty: 'oauth-authz-req+jwt' }).and.not.have.property('kid');
            });
        });
      }
